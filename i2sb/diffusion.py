...
class Diffusion():
    def __init__(self, betas, device):
        ...
        self.square_fwd = to_torch(square_fwd).to(device)
    ...
    def get_square_fwd(self, step,xdim=None):
        square_fwds = self.square_fwd[step]
        return square_fwds if xdim is None else unsqueeze_xdim(square_fwds, xdim)
    
    def get_std_sb(self,step, xdim=None):
        std_sb_step= self.std_sb[step]
        return std_sb_step if xdim is None else unsqueeze_xdim(std_sb_step, xdim)
    
    def get_std_fwd(self, step, xdim=None):
        std_fwd_step = self.std_fwd[step]
        return std_fwd_step if xdim is None else unsqueeze_xdim(std_fwd_step, xdim)

    def get_betas(self, step, xdim=None):
        betes_step= self.betas[step]
        return betes_step if xdim is None else unsqueeze_xdim(betes_step, xdim)
    ...
    def q_sample(self, step, x0, x1):
        ...
        μt = mu_x0 * x0 + mu_x1 * x1
        xt = μt + std_sb * torch.randn_like(μt)
        return μt.detach(),xt.detach(), std_sb
    ...
    def ddpm_sampling(self, steps, pred_x0_fn,pred_xt_noise, x1, mask=None, ot_ode=False, log_steps=None, verbose=True):
        ...
        for prev_step, step in pair_steps:
            ...
            μt = pred_xt_noise(xt,step)
            pred_x0 = pred_x0_fn(xt, step, μt)
